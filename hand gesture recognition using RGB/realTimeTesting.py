...
def masking(ind):
    ...
    while(m<5):
        ...
        fileName = './Dataa/RGB/Testframe{:d}.jpg'.format(frameNumd)
        cv2.imwrite(filename=fileName,img=rgbframes)
...
def Features():
    (FM, Files, FeatureNames) = getFeaturesFromDir("./Dataa/RGB")
    fo = open("./Dataa/Test/FeatureRGB/RGBfeatures", "w")
    cPickle.dump(FM, fo, protocol = cPickle.HIGHEST_PROTOCOL)
        

def Prediction():
    fv = cPickle.load(open("./Dataa/Test/FeatureRGB/RGBfeatures", "r+"))
    fv = np.array(fv)
    fv = fv.flatten()
